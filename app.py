...
# 2. PENGATURAN API KEY GOOGLE GEMINI
# Di Streamlit Cloud, kita akan menyetel ini secara aman di Advanced Settings.
# Kode di bawah ini mendeteksi API Key secara otomatis di server cloud.
if "GEMINI_API_KEY" in os.environ:
    st.sidebar.success("🔑 API Key terdeteksi dengan aman di Sistem.")
else:
...
if st.button("🚀 Mulai Analisis Karier Saya", type="primary"):
    ...
    if not os.environ.get("GEMINI_API_KEY"):
        st.error("Maaf, API Key Gemini belum diatur. Silakan masukkan di sidebar atau Advanced Settings.")
        ...
        with st.spinner("Tim AI sedang berdiskusi menganalisis kompetensimu... Mohon tunggu..."):
            try:
                # Inisialisasi Google GenAI Client resmi
                client = genai.Client()
                ...
                interaction_1 = client.interactions.create(model=MODEL_NAME, input=prompt_agent_1)
                hasil_agent_1 = interaction_1.output_text
                ...
                interaction_2 = client.interactions.create(model=MODEL_NAME, input=prompt_agent_2)
                hasil_agent_2 = interaction_2.output_text
                ...
                interaction_3 = client.interactions.create(model=MODEL_NAME, input=prompt_agent_3)
                hasil_agent_3 = interaction_3.output_text
                ...
                interaction_4 = client.interactions.create(model=MODEL_NAME, input=prompt_agent_4)
                hasil_akhir_bk = interaction_4.output_text
